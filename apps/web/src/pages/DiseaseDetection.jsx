...
export default function DiseaseDetection() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Page Header */}
      <div>
        <h1 className="font-headline font-extrabold text-2xl text-on-surface tracking-tight">Disease Detection 🔬</h1>
        <p className="font-label text-sm text-on-surface-variant/60 mt-1">AI-powered leaf analysis for instant disease diagnosis</p>
      </div>
      ...
      <div className="bg-primary/5 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="font-headline font-bold text-sm text-on-surface">Expert Tips for Best Results</p>
          <p className="font-label text-xs text-on-surface-variant/60 mt-1">
            📸 Use natural lighting • 🍃 Focus on the affected leaf area • 📐 Keep the leaf flat and centered
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl editorial-shadow overflow-hidden">
            <div className="p-5 bg-surface-container-low flex items-center justify-between">
              <h3 className="font-headline font-bold text-on-surface flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">photo_camera</span> Upload Leaf Image 📸
              </h3>
              <span className="smart-chip bg-primary/10 text-primary">AI Powered</span>
            </div>
            <div className="p-6 space-y-5">
              {/* Crop Selection */}
              <div className="space-y-2">
                <label className="font-label text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50">Select Crop Type</label>
                <div className="grid grid-cols-2 gap-3">
                  {crops.map(crop => (
                    ...
                    >
                      ...
                      <div className="font-headline font-bold text-sm">{crop.name}</div>
                      {!crop.available && <div className="font-label text-[10px] text-on-surface-variant mt-1">Model not loaded</div>}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Image Upload */}
              <div className="space-y-2">
                <label className="font-label text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50">Upload Leaf Image</label>
                <div
                  ...
                  {imagePreview ? (
                    <div className="text-center">
                      ...
                      <button onClick={e => { e.stopPropagation(); handleReset() }}
                        ...
                        <span className="material-symbols-outlined text-sm align-middle mr-1">close</span>Remove
                      </button>
                    </div>
                  ) : (
                    <div className="text-center">
                      ...
                      <p className="font-headline font-bold text-sm text-on-surface">Drop image here or click to browse</p>
            ...
            <div className="bg-white rounded-2xl editorial-shadow overflow-hidden">
              <div className="p-5 bg-surface-container-low">
                <h4 className="font-headline font-bold text-sm text-on-surface flex items-center gap-2">
                  <span className="material-symbols-outlined text-sm text-primary">info</span> Detectable Diseases
                </h4>
              </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              <div className="spinner" /><span className="font-label text-sm text-on-surface-variant">Analyzing leaf image with AI model...</span>
            </div>
          )}
          ...
          {!result && !loading && !error && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="font-headline font-bold text-lg text-on-surface mb-2">Upload a Leaf Image</h3>
              <p className="text-sm text-on-surface-variant/60 max-w-sm">Select your crop, upload a photo of the affected leaf, and our AI will detect the disease and suggest treatment.</p>
            </div>
          )}
            ...
            <div className="space-y-4 animate-fade-in-up stagger-children">
              ...
              <div className={`bg-white rounded-2xl editorial-shadow p-6 ${result.is_healthy ? 'ring-2 ring-primary/20' : 'ring-2 ring-tertiary/20'}`}>
                <div className="flex items-center gap-2 mb-4">
                  <span className="smart-chip bg-surface-container text-on-surface-variant">ANALYSIS RESULT</span>
                  <span className="smart-chip bg-primary/10 text-primary">AI Certified</span>
                </div>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    <h2 className="font-headline font-extrabold text-xl text-on-surface">{result.disease}</h2>
                    <p className="font-label text-xs text-on-surface-variant/60">Crop: {result.crop}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="font-label text-[10px] text-on-surface-variant/50 uppercase">confidence</div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl editorial-shadow overflow-hidden">
                <div className="p-5 bg-surface-container-low">
                  <h3 className="font-headline font-bold text-on-surface flex items-center gap-2">
                    <span className="material-symbols-outlined text-primary">medication</span> Treatment Plan 💊
                  </h3>
                </div>
                <div className="p-5 space-y-3">
                  <div className="bg-secondary-container/20 rounded-2xl p-4">
                    <div className="font-label text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/40 mb-1">Spray / Medicine</div>
                    <div className="font-headline font-bold text-on-surface">{result.treatment.spray}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-tertiary-fixed/20 rounded-2xl p-4">
                      <div className="font-label text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/40 mb-1">Dosage</div>
                      <div className="font-headline font-bold text-sm text-on-surface">{result.treatment.dosage}</div>
                    </div>
                    <div className="bg-primary/5 rounded-2xl p-4">
                      <div className="font-label text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/40 mb-1">Interval</div>
                      <div className="font-headline font-bold text-sm text-on-surface">{result.treatment.interval}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {result.all_predictions && (
                <div className="bg-white rounded-2xl editorial-shadow overflow-hidden">
                  <div className="p-5 bg-surface-container-low">
                    <h4 className="font-headline font-bold text-sm text-on-surface flex items-center gap-2">
                      <span className="material-symbols-outlined text-sm text-primary">analytics</span> All Predictions
                    </h4>
                  </div>
                  <div className="p-5 space-y-1">
                    {result.all_predictions.map((pred, i) => (
                      <div key={i} className="flex items-center gap-3 py-3">
                        ...
                        <span className="flex-1 font-label text-sm">{pred.disease}</span>
